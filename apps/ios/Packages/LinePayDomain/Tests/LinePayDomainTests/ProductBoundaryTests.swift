...
struct ProductBoundaryTests {
    private struct LegacyCalculationResult: Encodable {
        let agreementID: String
        let agreementVersion: String
        let components: [PayComponent]
        let total: Money
        let agreementSnapshots: [AgreementSnapshot]?
        ...
        init(_ result: CalculationResult) {
            ...
            total = result.total
            agreementSnapshots = result.agreementSnapshots
        }
    }
    ...
    @Test func legacySnapshotPolicyAndUnknownResultIdentityAreNotRelabeled() throws {
        ...
        let old = try JSONDecoder().decode(
            CalculationResult.self, from: JSONEncoder().encode(LegacyCalculationResult(calculated)))
        #expect(
            old.provenance == nil && old.total.amount == calculated.total.amount
                && old.total.currencyCode == calculated.total.currencyCode)
        #expect(
...
struct CalloutInteractionTests {
    ...
    @Test func newMinimumIntroducedDuringCalloutCannotBeSilentlyIgnored() throws {
        ...
        let call = try work(
            (2026, 9, 1, 23, 0), (2026, 9, 2, 1, 0), kind: .callout,
            calloutEventID: UUID())
