...
struct PayCalculatorInvariantTests {
    ...
    func dailyTierBoundary(_ hours: Int) throws {
        ...
        #expect(
            result.components.allSatisfy {
                $0.workIntervalID == shift.id && !$0.explanation.isEmpty
            })
    }
    ...
    @Test func inputOrderDoesNotChangeSemanticOutput() throws {
        ...
        let rules = try agreement(dailyOvertimeTiers: [
            DailyOvertimeTier(afterHours: 8, multiplier: 2)
        ])
        let forward = try PayCalculator().calculate(
            work: [a, b], agreement: rules, policy: .highestApplicable)
        let reverse = try PayCalculator().calculate(
        ...
        #expect(
            forward.components.map(\.workIntervalID) == reverse.components.map(\.workIntervalID))
        #expect(
            try forward.components.reduce(Money.zero(currencyCode: "USD")) {
    ...
    @Test func dailyOvertimeResetsAtMidnight() throws {
        ...
        let result = try PayCalculator().calculate(
            work: [shift],
            agreement: agreement(dailyOvertimeTiers: [
                DailyOvertimeTier(afterHours: 8, multiplier: 2)
            ]),
    ...
    @Test func actualCalloutBreakAndGuaranteedPayStaySeparate() throws {
        ...
        #expect(result.total.amount == 160)
        #expect(
            result.components.filter { $0.category == .workedHours }.reduce(Decimal(0)) {
                $0 + ($1.hours ?? 0)
            } == decimal("1.5"))
        #expect(
            result.components.first { $0.category == .calloutGuarantee }?.hours == decimal("2.5"))
    }
    ...
    @Test func breakCoveringMidnightDoesNotCreatePhantomWorkDate() throws {
        ...
        let shift = try WorkInterval(
            startEpochSeconds: start, endEpochSeconds: start + 14_400,
            timeZoneIdentifier: "UTC", unpaidBreaks: [pause])
        let result = try PayCalculator().calculate(
            work: [shift],
            agreement: agreement(
                flatPerDiem: FlatPerDiemRule(
                    amountPerWorkDate: Money(amount: 100, currencyCode: "USD"))),
            policy: .highestApplicable)
    ...
    @Test func effectiveDateBoundsAreInclusive() throws {
        ...
        let result = try PayCalculator().calculate(
            work: [valid], agreement: rules, policy: .highestApplicable)
        ...
        #expect(
            throws: PayCalculationError.workOutsideAgreementEffectiveDates(
                LocalDate(year: 2026, month: 8, day: 4))
        ) {
            try PayCalculator().calculate(
                work: [invalid], agreement: rules, policy: .highestApplicable)
        }
    }
