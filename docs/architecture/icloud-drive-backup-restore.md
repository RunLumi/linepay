...
Excluded: bytes still being read before an import is staged, deliberately deleted originals, temporary exports, StoreKit transactions/Pro entitlement, and device/account configuration. Old JSON-only exports are not complete backups and are rejected by this restore path rather than silently restoring without evidence.
...
- After commit, delete prior referenced originals. A cleanup error is a successful restore with a warning, not a false failed-restore message. Interrupted processes may leave unreferenced originals; they must not leave committed records pointing at uninstalled originals. On reload, app-generated UUID files without live references are queued for retryable cleanup. Delete all local data removes app-owned originals and preserves unrelated filenames and user-exported copies.
