...
| ReportShareSession.swift | ReportShareSessionTests; ReportTransferTests | Preview-bound immutable bytes, current-file readback, symlink replacement rejection, and temporary-export cleanup. The Files receiver still requires native UI validation. |
| ReportSharingView.swift | LegalConsentAndReportTests; LegalJourneyTests | Default-private report options, consent and scope state, and the real system share/save journey. Native activity-sheet behavior remains device/OS evidence. |
| TemporaryExports.swift | ReportShareSessionTests; ReportTransferTests | UUID-scoped temporary report URLs, exact preview/file parity, and bounded cleanup. Files/iCloud transfer is not proved by unit tests. |
